            ...
            {activeTab === 'overview' ? (
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div className="card p-6 bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all rounded-3xl group">
                            <div className="flex justify-between items-start mb-4">
                                <div className="p-3 bg-brand-50 text-brand-600 rounded-2xl group-hover:bg-brand-600 group-hover:text-white transition-all">
                                    <DollarSign size={24} />
                                </div>
                                <span className="text-xs font-black text-green-500 bg-green-50 px-2 py-1 rounded-full">+12.5%</span>
                            </div>
                            <h4 className="text-slate-400 text-xs font-black uppercase tracking-widest">Global Revenue</h4>
                            <p className="text-2xl font-black mt-1 tracking-tighter">${stats?.totalRevenue?.toLocaleString()}</p>
                        </div>
                        ...
                        <div className="card p-6 bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all rounded-3xl group">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className="text-xs font-black text-blue-500 bg-blue-50 px-2 py-1 rounded-full">LIVE</span>
                            </div>
                            <h4 className="text-slate-400 text-xs font-black uppercase tracking-widest">Total Orders</h4>
                            <p className="text-2xl font-black mt-1 tracking-tighter">{stats?.totalOrders?.toLocaleString()}</p>
                        </div>
                        ...
                        <div className="card p-6 bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all rounded-3xl group">
                            ...
                            <h4 className="text-slate-400 text-xs font-black uppercase tracking-widest">Today's Revenue</h4>
                            <p className="text-2xl font-black mt-1 tracking-tighter">${stats?.todayRevenue?.toLocaleString()}</p>
                        </div>
                        ...
                        <div className="card p-6 bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all rounded-3xl group">
                            ...
                            <h4 className="text-slate-400 text-xs font-black uppercase tracking-widest">Today's Orders</h4>
                            <p className="text-2xl font-black mt-1 tracking-tighter">{stats?.todayOrders}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        {/* Revenue Over Time */}
                        <div className="card p-8 bg-white border border-slate-100 shadow-xl rounded-[40px]">
                            <div className="flex justify-between items-center mb-8">
                                <div>
                                    <h3 className="text-lg font-black text-slate-900 tracking-tight">REVENUE_GROWTH_MATRIX</h3>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Last 7 days performance</p>
                                </div>
                        ...
                        <div className="card p-8 bg-white border border-slate-100 shadow-xl rounded-[40px]">
                            <div className="flex justify-between items-center mb-8">
                                <div>
                                    <h3 className="text-lg font-black text-slate-900 tracking-tight">OPERATIONAL_STATUS_PIE</h3>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Order distribution metrics</p>
                                </div>
                                ...
                                <div className="w-1/3 space-y-3">
                                    {stats?.ordersByStatus?.map((entry: any, index: number) => (
                                        <div key={entry._id} className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-tighter text-slate-600">{entry._id} ({entry.count})</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="card p-8 bg-slate-900 text-white rounded-[40px] shadow-2xl shadow-slate-200">
                        <div className="flex justify-between items-center mb-6">
                            <div className="flex items-center gap-3">
                                <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse shadow-[0_0_15px_rgba(34,197,94,0.6)]"></div>
                                <h3 className="text-lg font-black tracking-tight uppercase">Live_Transaction_Stream</h3>
                            </div>
                            <button className="text-[10px] font-black text-slate-400 hover:text-white border border-slate-700 px-4 py-1 rounded-full tracking-widest transition-all">
                                CLEAR.LOGS
                            </button>
                        </div>
